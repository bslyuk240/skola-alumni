    ...
    <main className="flex-1 px-4 py-4 sm:px-6 sm:py-6">
      ...
      {rows.length === 0 ? (
        <div className="mt-4 rounded-lg border border-neutral-100 bg-white p-6 text-center shadow-sm">
          <p className="text-sm text-neutral-500">No members match your search.</p>
        </div>
      ) : (
        <>
          {/* Card list below md — a 5-column table doesn't fit a phone screen. */}
          <ul className="mt-4 flex flex-col gap-3 md:hidden">
            {rows.map((row) => (
              <li key={row.membershipId} className="rounded-lg border border-neutral-100 bg-white p-4 shadow-sm">
                ...
                <p className="mt-2 truncate text-sm text-neutral-700">{row.email}</p>
                <p className="text-xs text-neutral-500">
                  Joined {row.approvedAt ? new Date(row.approvedAt).toLocaleDateString("en-NG") : "—"}
                </p>
                <div className="mt-3">
                  <SuspendToggle
